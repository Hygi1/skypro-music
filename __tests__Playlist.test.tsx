...
test("Playlist renders tracks", () => {
  ...
  const trackTitle = screen.getByText("Chase", {
    selector: ".track__titleLink",
  });
  expect(trackTitle).toBeInTheDocument();
});

test("Playlist shows 'Нет треков' when tracks array is empty", () => {
  const store = createTestStore();
  render(
    <Provider store={store}>
      <Playlist tracks={[]} />
  ...
  expect(screen.getByText("Нет треков")).toBeInTheDocument();
});
